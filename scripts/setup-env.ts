...
//
// Variable handling:
// - AUTO_GENERATED: generated if missing, empty, or value contains "change-me" (or key-specific placeholders)
// - PROMPTS: user is prompted (or default/existing used with --yes); optional keys can be skipped with Enter
// - Default: keep existing value or use value from env.*.example
//

/**
 * Variables that should be automatically generated if missing or placeholder
 */
const AUTO_GENERATED: Record<string, () => string> = {
    ...
    // LibreChat hex-decodes both before use: AES-CBC needs a 32-byte key and an exactly
    // 16-byte IV. Too short and every encrypt() throws ("iv must contain exactly 16 bytes"),
    // which silently breaks per-user MCP credentials and OAuth token storage.
    'LIBRECHAT_CREDS_KEY': () => genSecret(32), // 64 hex chars = 32 bytes
    'LIBRECHAT_CREDS_IV': () => genSecret(16),  // 32 hex chars = 16 bytes
 ...
 * Generates when: missing, empty, value contains "change-me", or value is a key-specific placeholder.
 */
/**
 * Secrets whose length is part of the contract, in hex characters. A value of the wrong length is
 * not a working secret - it makes the consumer throw - so it gets regenerated like a placeholder.
 * Only for values that cannot have been used successfully, otherwise regenerating loses data.
 */
const REQUIRED_HEX_LENGTHS: Record<string, number> = {
    'LIBRECHAT_CREDS_KEY': 64,
    'LIBRECHAT_CREDS_IV': 32,
};
...
function processAutoGenerated(
    ...
    const placeholders = AUTO_GENERATE_PLACEHOLDERS[key];
    const requiredLength = REQUIRED_HEX_LENGTHS[key];
    const isWrongLength = requiredLength !== undefined && !!currentValue && currentValue.length !== requiredLength;
    const isPlaceholder =
        !currentValue ||
        currentValue === 'change-me' ||
        currentValue.includes('change-me') ||
        (placeholders && placeholders.includes(currentValue));
    if (isPlaceholder || isWrongLength) {
        const secret = generator();
        console.log(
            isWrongLength
                ? `✨ Regenerated ${key}: was ${currentValue?.length} hex chars, needs ${requiredLength}`
                : `✨ Generated new secret for ${key}`
        );
        return secret;
    }
    return currentValue;
