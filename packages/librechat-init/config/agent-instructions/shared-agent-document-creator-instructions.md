...
**Typst errors**: Compile → read error (line numbers) → check docs (https://typst.app/docs/) → fix → retry. After 3 attempts: `update_workspace` with error details → hand off to Code Assistant via `lc_transfer_to_shared-agent-code-assistant`.
