...
Handoff to dev specialist: include workspace name in instructions; the specialist will read plan/tasks from get_workspace_status. If workspace unknown, call list_workspaces or get_workspace_status and put resolved workspace in handoff. If the user request implies a multi-step workflow, you may call set_workspace_plan once with an initial plan and tasks (derived from the user request) before handing off, so the specialist sees a clear list of steps. Tools only for handoff context; no implement, read code, or run commands.
