/**
 * Tool: get_video_info
 * Metadata (title, duration, extractor) for a URL without downloading – preview before download.
 */
...
export interface GetVideoInfoDeps {
...
export async function getVideoInfo(
  input: unknown,
  deps: GetVideoInfoDeps,
): Promise<{ content: TextContent[] }> {
  const parsed = GetVideoInfoSchema.safeParse(input);
  if (!parsed.success) {
  ...
  try {
    ...
    const relay = 'Metadata above. Use request_video_transcript to start download/transcript.';
    const text = formatVideoInfoResponse({ title, duration, extractor, relay });
    return { content: [{ type: 'text', text }] };
  } catch (e) {
    const err = e instanceof Error ? e : new Error(String(e));
    logger.warn({ err, video_url }, 'YTPTube GET /api/yt-dlp/url/info failed');
    return {
      content: [
        { type: 'text', text: formatErrorResponse(`Failed to get video info: ${err.message}`) },
      ],
    };
  }
}
