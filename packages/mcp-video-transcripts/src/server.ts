...
/** MCP Video Transcripts: video URL → transcript (YTPTube + Scaleway STT). Streamable-http. */
...
const PORT = parseInt(
  process.env.MCP_VIDEO_TRANSCRIPTS_PORT ?? process.env.PORT ?? '3010',
  10,
);
const SERVER_NAME = 'mcp-video-transcripts';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const server = app.listen(PORT, '0.0.0.0', () => {
      logger.info({ port: PORT, service: SERVER_NAME, version: SERVER_VERSION }, 'MCP Video Transcripts Server started');
    });
