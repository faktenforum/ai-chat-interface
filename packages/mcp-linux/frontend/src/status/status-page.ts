import { getAuthToken, postJson, formatBytes } from '../shared/http';
import { $, escapeHtml, setBanner } from '../shared/dom';
import type { OverviewResponse } from '../shared/types';

const userCardBody = document.querySelector<HTMLElement>('#userCard .card-body');
const workspacesCardBody = document.querySelector<HTMLElement>('#workspacesCard .card-body');
const uploadsCardBody = document.querySelector<HTMLElement>('#uploadsCard .card-body');
const downloadsCardBody = document.querySelector<HTMLElement>('#downloadsCard .card-body');
const terminalsCardBody = document.querySelector<HTMLElement>('#terminalsCard .card-body');
...
type StatusOverview = OverviewResponse & {
  workspaces: string[];
...
function renderUser(user: OverviewResponse['user']): void {
  ...
  let html =
    ...
    (user.username ? '<span class="badge">User: ' + escapeHtml(user.username) + '</span>' : '') +
    (user.diskUsage ? '<span class="badge">Disk: ' + escapeHtml(user.diskUsage) + '</span>' : '') +
    (user.home ? '<span class="badge">Home: ' + escapeHtml(user.home) + '</span>' : '') +
    '</div>';
  ...
  if (runtimeKeys.length > 0) {
    ...
    for (const key of runtimeKeys) {
      html +=
        ...
        escapeHtml(key) +
        ': ' +
        escapeHtml(String(runtimes[key] || 'unknown')) +
        '</span>';
    }
    html += '</div>';
  }
...
function renderWorkspaces(workspaces: string[] | null | undefined): void {
  ...
  const items = workspaces
    .map((name) => {
      return (
        ...
        '<div class="list-item-header">' +
        '<div class="list-item-title">' +
        '<a class="workspace-link" href="/status/workspace/' +
        ...
        '</div>' +
        '<div>' +
        '<button class="pill-button" data-action="open-vscode" data-workspace="' +
        escapeHtml(name) +
        '" disabled>Open VS Code (coming soon)</button>' +
        '</div>' +
        '</div>' +
        '<div class="list-item-meta">' +
        (name === 'default'
          ? '<span class="badge">Default workspace</span>'
          : '<span class="badge">Custom workspace</span>') +
        '</div>' +
        '<div class="list-item-meta">' +
        (name === 'default'
          ? ''
    ...
    .join('');
  workspacesCardBody.innerHTML = '<div class="list">' + items + '</div>';
}

function renderUploadSessions(sessions: UploadSession[] | null | undefined): void {
  ...
  const items = sessions
    .map((s) => {
      ...
      return (
        ...
        '<div class="list-item-header">' +
        '<div class="list-item-title">' +
        escapeHtml(s.workspace) +
        '</div>' +
        '<span class="badge ' +
        (s.status === 'completed'
          ? 'badge-success'
          : s.status === 'expired' || s.status === 'closed'
          ? 'badge-error'
          : '') +
        '">' +
        escapeHtml(s.status) +
        ...
        '<div class="list-item-meta">' +
        '<span>Token: ' +
        escapeHtml(String(s.token).slice(0, 8)) +
        '…</span>' +
        '<span>Expires: ' +
        escapeHtml(s.expires_at) +
        '</span>' +
        (s.uploaded_file
          ? '<span>File: ' +
            escapeHtml(s.uploaded_file.name) +
            ' (' +
            formatBytes(s.uploaded_file.size) +
            ')</span>'
          : '') +
        '</div>' +
        '<div class="list-item-meta">' +
        (canClose
          ? '<button class="pill-button" data-action="close-upload" data-token="' +
            escapeHtml(s.token) +
    ...
    .join('');
  uploadsCardBody.innerHTML = '<div class="list">' + items + '</div>';
}

function renderDownloadSessions(sessions: DownloadSession[] | null | undefined): void {
  ...
  const items = sessions
    .map((s) => {
      ...
      return (
        ...
        '<div class="list-item-meta">' +
        (canClose
          ? '<button class="pill-button" data-action="close-download" data-token="' +
            escapeHtml(s.token) +
            '">Revoke link</button>'
          : '') +
        '</div>' +
    ...
    .join('');
  downloadsCardBody.innerHTML = '<div class="list">' + items + '</div>';
}

function renderTerminals(terminals: TerminalInfo[] | null | undefined): void {
  ...
  const items = terminals
    .map((t) => {
      ...
      return (
        '<div class="list-item">' +
        '<div class="list-item-header">' +
        '<div class="list-item-title">Terminal ' +
        escapeHtml(id) +
        '</div>' +
        '</div>' +
        '<div class="list-item-meta">' +
        (workspace ? '<span>Workspace: ' + escapeHtml(workspace) + '</span>' : '') +
        (cwd ? '<span>CWD: ' + escapeHtml(cwd) + '</span>' : '') +
        '</div>' +
        '<div class="list-item-meta">' +
        '<button class="pill-button" data-action="kill-terminal" data-terminal-id="' +
        escapeHtml(id) +
        '">Kill</button>' +
        '</div>' +
        '</div>'
    ...
    .join('');
  terminalsCardBody.innerHTML = '<div class="list">' + items + '</div>';
}
...
function refreshOverview(): void {
  ...
  fetch(overviewUrl())
    .then((res) => {
      ...
      return res.json() as Promise<StatusOverview>;
    })
    .then((data) => {
...
function closeUploadSession(token: string): void {
  banner('info', 'Closing upload session...');
  postJson('/status/api/close-upload-session', { token })
    .then((res) => {
      if (!res.ok) {
        return res
          .json()
    ...
    .catch((err: unknown) => {
      ...
      banner('error', msg || 'Failed to close upload session');
    });
}

function closeDownloadLink(token: string): void {
  banner('info', 'Revoking download link...');
  postJson('/status/api/close-download-link', { token })
    .then((res) => {
      if (!res.ok) {
        return res
          .json()
    ...
    .catch((err: unknown) => {
      const msg =
        err && typeof err === 'object' && 'message' in err ? String((err as any).message) : undefined;
      banner('error', msg || 'Failed to revoke download link');
    });
}
...
function killTerminal(terminalId: string): void {
  ...
  postJson('/status/api/kill-terminal', { terminal_id: terminalId })
    ...
    .catch((err: unknown) => {
      const msg =
        err && typeof err === 'object' && 'message' in err ? String((err as any).message) : undefined;
      banner('error', msg || 'Failed to kill terminal');
    });
}
