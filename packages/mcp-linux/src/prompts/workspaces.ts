...
**Recommended flow:** After a handoff → \`get_workspace_status(workspace)\` (workspace from handoff instructions; use \`default\` if none given). Plan and tasks are the **source of truth** for what to do next. Need a workspace? → \`list_workspaces\` first. Before creating one → \`list_workspaces\` to avoid "already exists". When handing off → **always** call \`set_workspace_plan\` first so the next agent has current state; then hand off with workspace name in instructions (optionally one short hint; do not duplicate the full plan/task list in handoff text).

Workspaces store a **plan** (goal/context) and **tasks** (concrete steps) as the single source of truth for continuity across handoffs.

- **Read:** \`get_workspace_status\` returns \`plan\`, \`tasks\`, and optionally \`instructions\` (from \`.mcp-linux/instructions.md\`) plus git status. Each task has \`title\` and \`status\`. **Status:** pending (not started), in_progress (you are working on it), done (finished), cancelled (skipped or abandoned). When you **start** or **receive a handoff**, call \`get_workspace_status\` for that workspace and follow the plan and tasks. For expert workspaces (e.g. data-analysis, document-creator), follow \`instructions\` when present. If there is no or empty plan/tasks after a handoff, set an initial plan and tasks with \`set_workspace_plan\` from the handoff instructions, then proceed. **Note:** File lists in status may be summarized/capped (\`staged_count\`, \`truncated\`); use \`read_workspace_file\` with explicit paths (e.g. from \`list_upload_sessions\`) for specific files. Uploads in \`uploads/\` are temporary and may be purged; move or download important outputs.
- **Write:** \`set_workspace_plan\` sets \`plan\` (stored in \`.mcp-linux/plan.md\`) and/or \`tasks\` (stored in \`.mcp-linux/tasks.json\`). \`instructions.md\` is not modified by tools. **Always** call it before every handoff or when finishing your part so the next agent has current state; if you omit this, context is lost. For status-only updates you can pass \`task_updates: [{ index: 0, status: 'done' }, ...]\` instead of the full tasks array. **Tasks:** Prefer an array of strings, e.g. \`["Step 1", "Step 2"]\` — each becomes a task with status pending. Or \`[{ title, status? }]\` for per-task status. When **handing off**, call \`set_workspace_plan\` first: mark completed tasks \`done\`, the next task \`in_progress\` or \`pending\`, optionally update the plan summary; then hand off with the workspace name in the handoff instructions (optional short hint only).
