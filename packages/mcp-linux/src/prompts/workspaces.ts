...
- **Write:** \`set_workspace_plan\` sets \`plan\` and/or \`tasks\` (you can update only plan, only tasks, or both). **Always** call it before every handoff or when finishing your part so the next agent has current state; if you omit this, context is lost. For status-only updates you can pass \`task_updates: [{ index: 0, status: 'done' }, ...]\` instead of the full tasks array. **Tasks:** Prefer an array of strings, e.g. \`["Step 1", "Step 2"]\` — each becomes a task with status pending. Or \`[{ title, status? }]\` for per-task status. When **handing off**, call \`set_workspace_plan\` first: mark completed tasks \`done\`, the next task \`in_progress\` or \`pending\`, optionally update the plan summary; then hand off with the workspace name in the handoff instructions (optional short hint only).
