...
All cryptographic secrets (session, JWT, CREDS_KEY/IV, Meili master key, etc.) are auto-generated by the setup script:

```bash
npm run setup:prod:yes    # generates .env.prod with fresh secrets
npm run setup:dev:yes     # generates .env.dev with fresh secrets
```

The script detects placeholder values from `env.local.example` (including well-formed hex placeholders like `0123...` for `LIBRECHAT_CREDS_KEY`) and replaces them with cryptographically random values. Secrets already set in an existing `.env.prod` / `.env.dev` are preserved on re-runs.

### Critical: LIBRECHAT_CREDS_KEY and LIBRECHAT_CREDS_IV

These two variables **must** be set before the first production deployment. They are used by LibreChat to encrypt user credentials (API keys, OAuth tokens) stored in MongoDB via AES-256.

| Variable | Length | Format | Used for |
|----------|--------|--------|----------|
| `LIBRECHAT_CREDS_KEY` | 64 hex chars | 32-byte AES-256 key | Encrypting stored credentials |
| `LIBRECHAT_CREDS_IV` | 32 hex chars | 16-byte initialization vector | Encrypting stored credentials |

**If these are missing or have placeholder values**, LibreChat will either crash on startup or silently fail to encrypt/decrypt user credentials. The `setup-env.ts` script generates them automatically — verify they are present in `.env.prod` before deploying:

```bash
grep "LIBRECHAT_CREDS_KEY\|LIBRECHAT_CREDS_IV" .env.prod
# Both lines should show non-placeholder hex values (64 and 32 chars respectively)
```

**⚠️ Never change CREDS_KEY/IV after users have stored credentials** — doing so makes all previously encrypted credentials undecryptable. If rotation is absolutely necessary, users must re-enter their API keys.

### Deploying secrets to Portainer

1. Generate `.env.prod` locally: `npm run setup:prod:yes`
2. In Portainer: Stack → **Editor** → **Environment variables** (Advanced mode)
3. Paste the entire contents of `.env.prod`
4. Deploy the stack

### Machine-user credentials (SSH key + PAT)

These are set via `MCP_LINUX_GIT_SSH_KEY` (base64 SSH private key) and `MCP_GITHUB_PAT` (GitHub PAT) in the same `.env.prod` / `.env.dev` file. See [GitHub Machine User](GITHUB_MACHINE_USER.md) for generation and rotation procedures.
