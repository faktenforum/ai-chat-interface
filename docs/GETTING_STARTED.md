...
The project provides four Docker Compose stacks:

1. **`docker-compose.local.yml`** - Local development with official images (`.env.local`)
2. **`docker-compose.local-dev.yml`** - Local development with builds from git submodules (`.env.local`)
3. **`docker-compose.prod.yml`** - Portainer production builds (uses `:latest` image tags)
4. **`docker-compose.dev.yml`** - Portainer dev/test builds for feature branches (uses `:dev` image tags)
...
#### Option 2: Local Development Builds (`docker-compose.local-dev.yml`)
...
**Available npm scripts:**
- `npm run build:local-dev` - Build Docker images from source
- `npm run rebuild:local-dev` - Rebuild images without cache
...
- **Normal restart** (preserves data): Use `npm run restart:local` or `npm run restart:local-dev`
...
**Stack Selection:**
- **Production**: Use `docker-compose.prod.yml` with `:latest` image tags (main branch builds)
- **Dev/Test**: Use `docker-compose.dev.yml` with `:dev` image tags (feature branch builds)
...
2. **Deploy in Portainer CE:**
   ...
   - Compose path: `docker-compose.prod.yml` (production) or `docker-compose.dev.yml` (dev/test)
...
**Important**: MailDev is **only** used for local development (`docker-compose.local.yml` and `docker-compose.local-dev.yml`). 
