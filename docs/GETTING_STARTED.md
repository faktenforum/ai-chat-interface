...
#### Test Environment (Portainer Deployment)
For deploying to a test environment on Portainer:

```bash
npm run setup:dev
# Then deploy via Portainer using docker-compose.yml and .env.dev contents
```
...
1. **Generate Environment**: Generate production environment file:
   ...
   npm run setup:prod
   ```
   This creates `.env.prod` with production-specific settings (no duplicates, production overrides applied).
