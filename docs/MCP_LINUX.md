...
- **SSH**: Optional `MCP_LINUX_GIT_SSH_KEY` (base64 ed25519 private key) → written to each user's `~/.ssh/id_ed25519` on account creation and on every container start, so a rotated key reaches existing accounts. Use same account as `MCP_GITHUB_PAT` (see [GitHub Machine User](GITHUB_MACHINE_USER.md)).
- **Host keys**: github.com is verified against the keys shipped in `src/utils/github-known-hosts.ts` (`StrictHostKeyChecking yes`, `~/.ssh/known_hosts_github`, rewritten with the key). The default `~/.ssh/known_hosts` stays the user's, for any other host they add.
- **GitHub CLI**: Optional `MCP_GITHUB_PAT` → authenticates `gh` CLI for all users (PR creation, GitHub operations). Same PAT as GitHub MCP for consistency. Written to `~/.config/gh/hosts.yml` when the account is created and on the user's first request after a restart.
...
- The token arrives with every request; a change is applied on the next one, and unchanged requests touch no files. A container start writes no GitHub credentials at all - it has no per-user token to write, and the shared one would land on top of the configuration of everyone who has their own. The first request of each user applies whichever token belongs to them, so a rotated shared token also arrives with the next message.
- **No token at all**: `get_status` reports the credential state (`github.token`: `user` / `shared` / `none`) with a note naming the LibreChat setting, and a clone that fails on authentication gets the same note appended - instead of the model explaining a raw `Permission denied (publickey)` on its own.
