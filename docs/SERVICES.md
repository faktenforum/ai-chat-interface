...
| **Spend Monitor** | Org cost dashboard, per-user limit resets + admin-gated MCP endpoint (`/mcp`); reads LibreChat MongoDB | ✅ `http://spend.localhost` | ✅ `https://spend.{DOMAIN}` (basic auth) | ❌ |
...
| **Mail** | Internal Docker | 3015 | `http://mcp-linux:3015/mcp/mail` | ❌ | ✅ `localhost:3015` |
| **Calendar** | Internal Docker | 3015 | `http://mcp-linux:3015/mcp/calendar` | ❌ | ✅ `localhost:3015` |
...
**Mail** — The user's own mailbox over IMAP/SMTP, served by the same container at `/mcp/mail` but listed in LibreChat as its own server, because credentials are gated per server and mail needs them. Tools: `list_mailboxes`, `list_messages`, `search_messages`, `read_message`, `send_message`, `set_message_flags`, `move_message`, `delete_message`, `save_attachment`. Credentials are per user (`MAIL_ADDRESS`, `MAIL_PASSWORD`, optional host overrides) from LibreChat's encrypted store; the operator only sets the default hosts `MCP_MAIL_IMAP` and `MCP_MAIL_SMTP`. Attachments are saved into the Linux workspace, so the workspace tools can work on them. [MCP Linux](MCP_LINUX.md#mail-imapsmtp-at-mcpmail)

**Calendar** — The user's calendars over CalDAV, served by the same container at `/mcp/calendar` and listed in LibreChat as its own server. Tools: `list_calendars`, `list_events`, `read_event`, `create_event`, `update_event`, `delete_event`, `find_free_time`. Standard CalDAV with the usual discovery chain, so Nextcloud, Radicale and Baikal all work from the instance base URL alone; recurring events are expanded per occurrence. Credentials are per user (`CALDAV_USERNAME`, `CALDAV_PASSWORD`, optional `CALDAV_URL`) from LibreChat's encrypted store; the operator only sets `MCP_CALDAV_URL`. [MCP Linux](MCP_LINUX.md#calendar-caldav-at-mcpcalendar)
